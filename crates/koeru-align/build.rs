//! submodule の Kaldi（`vendor/kaldi`）から、**GMM 推論に要る部分だけ**をビルドする。
//!
...
//! なったとき」に直ちに当たる）。**KOERU が使うのは強制アライメントだけ**なので、
//! 必要な8モジュールを `cc` で直接組む。
//!
//! # OpenFst を引かない
//!
...
//!   typedef を使う。**上流自身が用意している分岐**なので、定義すれば OpenFst が要らない
...
//! `koeru-audio` と同じく、**書いていない OS では素直に落ちる**ようにしてある。
