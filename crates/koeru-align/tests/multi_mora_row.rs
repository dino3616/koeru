//! 録音リストが実際に生成する行でアライメントが通ることを確かめる
...
/// 退避経路は多モーラ行を名指しで断る（`DEC-ALN-015`）。
///
/// Julius を採らないので、連続音・CVVC では MFA が必須になる。
/// そのことが呼び出し側に分かる形で返らないと、モデルの不在と方式の不一致が
/// 同じ顔をする。
#[test]
fn 退避経路は多モーラ行を名指しで断る() {
    use koeru_align::aligner::{AlignError, AlignRequest, Aligner as _};
    use koeru_align::segment::HeuristicAligner;

    let a = HeuristicAligner::new("heuristic@test");
    let samples = vec![0.0_f64; 44100];
    let long = phoneme::phonemes_for_all(&["か", "き", "く"]).expect("引ける");
    let err = a
        .align(&AlignRequest {
            samples: &samples,
        ...
        .expect_err("多モーラは扱えない");
    assert!(
        matches!(err, AlignError::MultiMoraUnsupported),
        "音素列が空と同じ顔をしない: {err:?}"
    );
    assert_eq!(err.kind(), "align.multi_mora_unsupported");

    // 単独音は通る。 退避が全部を断るわけではない。
    let one = phoneme::phonemes_for_all(&["か"]).expect("引ける");
    assert!(
        !matches!(
            a.align(&AlignRequest {
                samples: &samples,
                sample_rate_hz: 44100,
                phonemes: &one,
                grid: None,
            }),
            Err(AlignError::MultiMoraUnsupported)
        ),
        "単独音まで断らない"
    );
}
