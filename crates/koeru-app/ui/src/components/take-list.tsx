 ...
 */
export const TakeList = ({ revision, busy, onRetake }: TakeListProps) => {
  ...
  return (
    <Card>
      ...
      {recorded.length === 0 ? (
        <p className="mt-3 text-sm text-text-dim">まだ1つも録れていません。</p>
      ) : (
        <ul className="mt-3 flex flex-col gap-2">
          {recorded.map((r) => (
            <li
              key={r.row_id}
              className="flex flex-wrap items-center gap-x-3 gap-y-2 border-border border-b py-2 last:border-b-0"
            >
              <span className="w-14 shrink-0 font-mono text-sm text-text-dim">{r.row_id}</span>
                ...
                {r.takes.map((t) => {
                  ...
                  return (
                    <Button
                      ...
                      disabled={busy || t.invalid || adopted}
                      onClick={() => adopt(r.row_id, t.take_id)}
                      title={
                        t.invalid
                          ? "取りこぼしがあったので使えません"
                          : `${(t.duration_ms / 1000).toFixed(1)} 秒`
                      }
                    >
                      {t.generation}
                      {t.invalid ? "✕" : ""}
