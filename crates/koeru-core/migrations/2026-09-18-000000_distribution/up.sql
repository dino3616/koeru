...
--
-- 1プロジェクトに1行ずつ。 id を 1 に固定して、2行目を作れなくする。
-- 音源は「プロジェクト1本 = 音源1本」なので、複数行は意味を持たない。
--
-- 表示名はここに置かない。 manifest.toml が持っている（TR-PKG-38）。
-- 配布名は表示名と別（DEC-PKG-008）ので、こちらに置く。
--
-- 2つに分けているのは、diesel の既定が1表 16 列までだから。
-- 分け方は配布物の側に合わせた——character.txt / character.yaml に出るものと、
-- readme.txt に出るもの。
...
CREATE TABLE distribution_readme (
  id INTEGER PRIMARY KEY CHECK (id = 1),
