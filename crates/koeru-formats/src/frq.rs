//! 周波数表（`.frq`）の書式（`TR-PKG-05`, `TR-SYN-23`, `TR-SYN-24`）。
//!
...
//! この層が持つのは書式だけで、推定は `koeru-synth` が、ファイルへ書くのは
//! `koeru_core::frq` が持つ。
//!
//! # 書式（FREQ0003）
//!
...
/// 周波数表を作るときの失敗。 ファイルへ書く失敗は `koeru_core::frq::FrqWriteError`。
