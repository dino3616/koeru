...
- **画面へ流し続けるものは Tauri の Channel で送る。`invoke` で引きに行かせない**（`DEC-PLT-017`）。公式が「Channels are designed to be fast and deliver ordered data」と書いていて、**`invoke` は応答の順序を保証しない**——遅れて届いた古いフレームを描くと波形が巻き戻る。event（`emit`/`listen`）も「not designed for low latency or high throughput」。**流し続けるものはアプリの状態ロックの外から読む**（テイク確定中は数秒握られる）
...
- **リングの位置は総数で持つ。剰余で持たない**（`DEC-REC-007`）。`% cap` した値の差を `wrapping_sub(..) % cap` で取ると、**`2^64 % cap == 0`——容量が2の冪のときにしか合わない。**アプリの容量 384000 では 111616 ずれ、**環をまたぐたびに 2.3 秒ぶんの古い音を読み直していた**（踏んだ）。**リングの試験を2の冪の容量だけで書かない**
