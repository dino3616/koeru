...
- **参照した入力ソース**: [product-vision.md](./product-vision.md)、[personas.md](./personas.md)、[journey-map.md](./journey-map.md)、既存ツールの情報構造（`EVID-UX-002`）、制作者の公開記録（`EVID-UX-003`）、現行実装の棚卸し（`EVID-UX-001`）
...
> **確度は動かしていない。** 既存ツールの情報構造と制作者の公開記録を足したが、どちらも確度が Assumption なので、これを根拠に `Assumption` を `Fact` へ上げることはしない（`meta/README.md` の「確度は下げる方向にしか動かない」）。**新しい出典が変えたのは、障壁の裏付けの厚さであって、ユースケースの検証状態ではない。**
...
|---|---|---|---|---|
| F01 | 声を録り始める | 手作業の有無で方式を選び、用語を覚えずに録音に入れる | ミナ / ハル | 6 |
...
| UC-F01-06 | F01 | 声を録り始める | 録り直しても、前に録ったものを失わない | ミナ / ハル | 録音の途中 | 「やり直した」が「失った」にならない | Assumption |
| UC-F02-01 | F02 | 録った声で歌わせる | 全部録り終える前に自分の声で歌を聴ける | ミナ / ハル | 初回録音の直後 | 「この声はこういう存在だ」と感じる | Assumption |
...
- **読み上げの難しさは、方式選択の主軸とは独立に効く。** CVVC と連続音の行は初心者には読み上げそのものが難しいという記録がある（`EVID-UX-003`）。方式の軸は1本のままにし、提示の設計で吸収する（`Q-REC-005`）。単独音は `TR-RCL-03` の行の長さで既に対処されている
- **UC-F01-01 の「説明を読まずに」の前に、準備の段がある。** 実装は最初のフレーズに着くまでに入力デバイスの選択・入力レベルの校正・音の回り込みの確認を通る（`EVID-UX-001`）。どれも要件が求めるもので消せない。**置き場所が UC-F01-01 の成否を左右する**
...
録音の途中でも、その時点の素材で歌を返す。プロダクトの中核はここにある。同時に、**その音源と初めて出会う場面**でもあるため、上手い / 下手の評価軸を持ち込まない（`docs/product-vision.md` の Where）。
...
- 試唱までの待ち時間の許容範囲は未確定
- **UC-F02-03 の「評価されずに」は、機材と設定の不備にも及ぶ。** 音量不足もクリッピングも指摘しないことを判断として確定している（`DEC-REC-008`）。上手い / 下手を持ち込まないだけでなく、**何が「不備」かを製品が決めない**。割れた音や小さい音を特徴として使う制作者がいるかもしれないため
...
- 録音時の品質判定（音量不足、クリッピング、声質の変化）は**範囲外ではなく、出さないことを選んでいる**（`DEC-REC-008`）。計測はしていて、クリッピングの判定規則も要件の中にある。失敗の発見は試唱まで遅れる
- **UC-F04-01 が省く時間の大きさは見積もれる。** 「原音設定は、ある程度習熟した後でも、連続音1音階あたり1〜2時間程度の作業時間が必要」と書いている記録がある（`EVID-UX-003`）。**習熟した後の数字**
- **収録項目が確認待ちという状態を持てないと UC-F04-02 は成立しない。** いまの収録項目が持つのは採用テイクの有無とテイクの世代だけで、「後で見直す」を置く場所が無い（`EVID-UX-001`）。既存ツールで項目ごとの状態をいちばん持っているのは vLabeler で、Done / Star / Tag をショートカット1打で切り替えられる（`EVID-UX-002`）
