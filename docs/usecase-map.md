...
- **実装の形**: 単一のネイティブアプリケーション。PC 前提。処理はローカル完結。まず UTAU エコシステムをサポートする
- **今回のモード**: 全体俯瞰
...
|---|---|---|---|---|
| F01 | 声を録り始める | 所要時間で方式を選び、用語を覚えずに録音に入れる | ミナ / ハル | 5 |
...
| UC-F01-03 | F01 | 声を録り始める | 方式ごとの所要時間を見て、どこまで作るか決められる | ミナ / ハル | 着手時 | 見積もりが立った状態で選べる | Assumption |
...
**図の読みどころ**: ソラは F02 と F05 にしか線が伸びていない。**「まず歌う」だけがソラにも等しく効き、完成の出口が公開でないルートを必要としている。** ミナとハルは F01 の `所要時間を見て方式を選ぶ` で同じノードを共有する。同じ画面を見て、ミナは8分を選び、ハルは3時間を選ぶ。**選択肢は同じで、選ぶものが違う。**
...
検索から来た人が、説明を読まずに最初のフレーズを録るところまでを担う。方式は最初に選ぶが、選択肢は方式名ではなく所要時間と到達点で示す。単独音なら約8分、多音階連続音なら約3時間。名前の入力は通過点であって、判断を求める場所にはしない。
...
- **所要時間の数値が未算出。** 単独音 約8分、多音階連続音 約3時間は目安として置いているが、その間にある方式（単一音階の連続音、CVVC）の値は要算出。**不正確だと判断材料として機能しない**
- ネイティブアプリなので、検索した端末（スマホ）と録る端末（PC）が違う。発見から着手までにインストールという段差が入る
