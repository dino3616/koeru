...
`docs/reports/ux/redesign-plan.md` の Phase 3。M7 まで見て骨格を決める。**この文書はまだオブジェクトと関連まで。** ビュー構成とナビゲーションは、ここが固まってから足す。
...
| **原音設定エントリ** | エイリアス1つ分の5値 | 収録音高 × エイリアス（`TR-ALN-22`） | 境界ごとに 信頼度 / 由来 / 確認状態 | `TR-EDT-01` `TR-EDT-02` `TR-PKG-19` `TR-ALN-22` |
| **曲** | 被覆の目標。同梱1〜2曲と、本人が持ち込む UST / USTX（USTX は1トラックが1曲） | 曲の識別子 | 完全 / 代替あり / 不可 | `TR-RCL-12` `TR-RCL-15` `TR-RCL-19` |
| **配布パッケージ** | 書き出した成果物。版の単位 | 方式 × プロファイル × 版 | 生成済み / 検証済み | `TR-PKG-12` `TR-PKG-24` `TR-PKG-44` `TR-PKG-46` |

原音設定エントリの同一性に収録音高が入るのは、多音階が音高ごとに独立した
oto の面を持つため。綴りだけを同一性にすると、音高の数だけある同じ綴りが
1つに潰れる。配布物の側では綴りが一意に戻る——区画の接頭辞・接尾辞が
音高を綴りへ織り込むので、`TR-PKG-19` の「音源全体（全 oto.ini 横断）で一意にする」は
そちらで満たされる。判断の経緯は `DEC-ALN-017`（同じ音高の中の持ち主の決め方は `DEC-RCL-016` が置き換えた）。
