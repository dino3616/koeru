...
| 部品台帳。1部品1行 | `evidence/components.toml` | `CMP-{番号}` |
...
**記録を足すときは、番号を `next-id` で取る。**

```bash
cargo xtask next-id DEC-PLT   # その接頭辞で、まだ使われていない番号を出す
```

ディレクトリを見て「たぶん次はこれ」と決めない。 既にあるファイルへ上書きすると、そのファイルが持っていた判断が消え、それを引用していた記録（`decided_by`）だけが残って別のことを指す。検査は素通りする——形も参照先も壊れていないので。**踏んだ。** `DEC-ALL-007`（署名配布と dlopen）を SPDX の判断で上書きし、`CMP-081` が挙げていた「ALSA へは dlopen 経由でのみ到達する」の根拠が消えた。

変更が触れた契約は `touched` が出す。 レビューの前に読む。
